...
如果你要表示为代码的单词或短语中包含一个或多个反引号，则可以通过将单词或短语包裹在反双引号（``）中。
...
- 表格的单元格宽度
    | 语法      | 描述         |
    | --------- | ----------- |
    | 宽度 | <div style="width: 100px;">这是一个很长的文本内容，会强制换行并占据指定宽度</div> |
    | 宽度 | <div style="width: 200px;">这是一个很长的文本内容，会强制换行并占据指定宽度</div> |
- 表格的单元格宽度。使用 table 标签设置整列宽度
    <table>
        <colgroup>
